...
class a3cSingleArm(mp.Process):
    def __init__(self, master, process_id=0):
        super(a3cSingleArm, self).__init__(name = "Process-%d" % process_id)
    ...
    def _preprocessState(self, state):
        #print(state)
        #print(self.master.dtype)
        state1 = np.array(state)
        state_ts = torch.from_numpy(state1).unsqueeze(0).type(self.master.dtype)
        return state_ts
...
class A3CLearner(a3cSingleArm):
    ...
    def _get_valueT_vb(self):
        if self.rollout.terminal1[-1]:  # for terminal sT
            ...
            sT_ts = torch.from_numpy(np.array(self.rollout.state1[-1])).unsqueeze(0).type(self.master.dtype)  # bootstrap from last state
            if self.master.enable_lstm:
...
class A3CEvaluator(a3cSingleArm):
    def __init__(self, master, process_id=0):
    ...
    def _eval_model(self):
        ...
        print("reward_avg: " + str(self.reward_avg_log[-1][1]))
